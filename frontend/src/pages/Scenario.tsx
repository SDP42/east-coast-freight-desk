        ...
        <div className="p-6">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label className="text-xs text-muted">
              Destination port
              <select className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-3 py-2 text-sm text-strong" value={portId ?? ""} onChange={(e) => setPortId(Number(e.target.value))}>
                {ports.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </label>
            <label className="text-xs text-muted">
              Cargo tonnage
              <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-3 py-2 text-sm text-strong" value={tonnes} onChange={(e) => setTonnes(e.target.value)} />
            </label>
          </div>
          ...
          <button onClick={run} disabled={loading || portId == null} className="mt-5 rounded-md bg-cyan/90 px-4 py-2 text-sm font-medium text-on-accent transition hover:bg-cyan disabled:opacity-50">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <SpotlightCard glowColor={result.best_origin_changed ? "251,191,36" : "52,211,153"}>
            <div className="p-5">
              ...
              <p className="mt-2 text-sm font-medium text-strong">{result.summary}</p>
            </div>
          </SpotlightCard>
                ...
                <tbody className="divide-y divide-border-soft/60">
                  {result.origins.map((o) => (
                    <tr key={o.origin_country} className="hover:bg-panel-light/60">
                      <td className="px-4 py-3 font-medium text-strong">{o.origin_country}</td>
                      <td className="px-4 py-3 text-body">{fmt(o.baseline_cost_usd)}</td>
                      <td className="px-4 py-3"><ArrowRight className="h-3.5 w-3.5 text-muted" /></td>
                      <td className="px-4 py-3 text-strong">{fmt(o.scenario_cost_usd)}</td>
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
                  {result.reroute_alternatives.map((r) => (
                    <div key={r.port_name} className="rounded-md border border-border-soft bg-panel-light/50 p-3 text-xs">
                      <p className="flex items-center gap-1.5 font-medium text-strong"><Waves className="h-3.5 w-3.5 text-cyan" />{r.port_name}</p>
