...
# ------------------------------------------------------------------ signals
@router.get("/signals/cyclone")
def cyclone(port: str, laycan_start: date, laycan_end: date, transit_days: float = 0.0, db: Session = Depends(get_db)) -> dict:
...
@router.get("/signals/berth-slots")
def berth_slots(port: str, db: Session = Depends(get_db)) -> dict:
    try:
        return cached(f"slots:{port}:{data_version(db, 'BDI')}:{date.today()}", 3600, lambda: signals.berth_slots(db, port))
    except ValueError as exc:
        raise _bad(exc)


@router.get("/signals/transfer")
def transfer(db: Session = Depends(get_db)) -> dict:
    return cached(f"transfer:{date.today()}", 3600, lambda: signals.congestion_transfer(db))


@router.get("/signals/demand")
...
# ------------------------------------------------------------------ forecasting extras
@router.get("/forecast-multi/{index_name}")
...
@router.post("/recommendation/pareto")
...
@router.get("/ledger")
def ledger_list(db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> dict:
    return {"entries": [_entry_out(e) for e in db.query(LedgerEntry).order_by(LedgerEntry.id.desc()).all()], "chain": ledger.verify_chain(db)}
...
@router.post("/ledger/sample")
def ledger_sample(db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    if db.query(LedgerEntry).filter(LedgerEntry.is_sample.is_(True)).count():
        raise HTTPException(status_code=409, detail="Sample entries are already loaded")
    return {"added": ledger.load_sample(db, user.id)}


@router.get("/ledger/benchmark")
def ledger_benchmark(db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> dict:
    rows = ledger.benchmark(db)
    ...
    return {"rows": rows, "summary": {"fixtures": len(rows), "avg_missed_saving_pct": round(sum(timed) / len(timed), 1) if timed else None},
            "method": "Timing: the real BDI on the fixture date against its trailing 90 days and the cheapest day within 30 days either side. 'Missed saving' assumes the rate moves in proportion to the BDI, which is a simplification. Rate check: your rate against the platform's illustrative rate for the route."}


# ------------------------------------------------------------------ monitoring
@router.get("/monitor/{index_name}")
...
@router.post("/monitor/{index_name}/retrain")
...
@router.get("/briefing")
def get_briefing(db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> dict:
    return cached(f"briefing:{data_version(db, 'BDI')}:{date.today()}", 1800, lambda: briefing.build(db))
