...
| Deep learning | PyTorch GRU trained offline (`scripts/train_current_dl.py`), result files served | Offline (results only only) |
...
| XGBoost | Gradient-boosted trees on lag, rolling, calendar and exogenous features (dollar index, oil, coal and freight price indices, rupee). |
...
| PyTorch (offline) | Trains the GRU comparison in `scripts/train_current_dl.py`. It is not a runtime dependency. |

## Data sources and licences

**Public-domain data only, fetched free with no account, key or credit.** Full detail and the list of sources removed for licence reasons: [LICENCES.md](LICENCES.md).

| Data | Source | Terms |
|---|---|---|
| Monthly grain ocean freight rates, US Gulf and PNW to Japan, 1996 to 2026 | USDA Agricultural Marketing Service, Grain Transportation Report | US government, non-copyrighted (rates compiled by O'Neil Commodity Consulting, credited) |
| Deep-sea freight and coal producer price indices | US Bureau of Labor Statistics via FRED | Public domain |
| Brent crude spot price | US Energy Information Administration via FRED | Public domain |
| Rupee, Australian dollar, rand, broad dollar index | Federal Reserve H.10 via FRED | Public domain |
| Cyclone tracks | NOAA IBTrACS | Public domain |
| Land outlines for the globe | Natural Earth 110m | Public domain |
| Port turnaround, import share, demurrage cases | Ministry of Ports, SAIL annual reports, CAG audit | Public Indian-government documents, quoted with citations |
| Ship availability | Broker open-tonnage lists uploaded by users | The organisation's own data |
...
BAAI/bge-small-en-v1.5 (MIT) through fastembed (Apache 2.0) improves the assistant's intent accuracy from 73.5% to 86.9%. It downloads once and runs locally with no key.
