...
def list_events(db: Session = Depends(get_db)) -> list[DisruptionEvent]:
    return db.query(DisruptionEvent).order_by(DisruptionEvent.start_date.desc()).all()


@router.get("/score", response_model=RouteRiskOut)
def get_route_risk(origin_country: str, destination_port_id: int, db: Session = Depends(get_db)) -> RouteRiskOut:
