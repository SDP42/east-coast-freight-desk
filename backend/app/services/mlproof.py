...
def live_refit(db: Session, index_name: str = "BPI", horizon: int = 7, splits: int = 10) -> dict:
    from statsmodels.tsa.arima.model import ARIMA
    from xgboost import XGBRegressor

    s = _series(db, index_name)
    if len(s) < 400:
    ...
    ar_t = xgb_t = 0.0
    origins = [int(x) for x in np.linspace(len(s) - horizon - 600, len(s) - horizon - 1, splits)]  # spread over the last ~2.5 years
    for o in origins:
    ...
    return {
        "index": index_name, "horizon_days": horizon, "walk_forward_origins": len(origins), "history_points": len(s),
...
def serving_map() -> list[dict]:
    return [
        ...
        {"result": "Deep models (LSTM, GRU, Transformer, TCN)", "how": "Trained offline with PyTorch (scripts/train_dl.py); the saved weights are read and run forward with NumPy in milliseconds. Weights below.", "live": False},
...
def dl_summary() -> dict | None:
    f = ART / "dl_results.json"
    return json.loads(f.read_text()) if f.exists() else None


def proof(db: Session, index_name: str = "BPI") -> dict:
