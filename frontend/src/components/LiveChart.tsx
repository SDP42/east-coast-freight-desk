      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <div className="flex items-baseline gap-3">
            <span key={cursor} className={`rounded px-1 text-3xl font-bold tabular-nums text-white ${tickUp ? "flash-up" : "flash-down"}`}>
        ...
        {!compact && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-md border border-border-soft bg-panel-light text-white hover:border-cyan/40"
            ...
            {SPEEDS.map((s) => (
              <button
                key={s}
                onClick={() => setSpeed(s)}
                className={`h-8 rounded-md border px-2.5 text-xs ${speed === s ? "border-cyan/50 bg-cyan/10 text-cyan" : "border-border-soft bg-panel-light text-muted hover:text-white"}`}
              >
                {s}x
              </button>
            ))}
      ...
      <div className="mt-3">
        <ResponsiveContainer width="100%" height={height}>
          <AreaChart data={visible} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            {!compact && <CartesianGrid strokeDasharray="3 3" stroke="#232d45" />}
            <XAxis dataKey="date" hide={compact} tick={{ fontSize: 10, fill: "#8792a8" }} tickLine={false} axisLine={{ stroke: "#232d45" }} minTickGap={40} />
            <YAxis hide={compact} domain={[(min: number) => min * 0.97, (max: number) => max * 1.03]} tick={{ fontSize: 10, fill: "#8792a8" }} tickLine={false} axisLine={false} width={48} tickFormatter={(v: number) => v.toLocaleString(undefined, { maximumFractionDigits: 0 })} />
            {!compact && <Tooltip contentStyle={{ background: "#131a2b", border: "1px solid #232d45", borderRadius: 8, fontSize: 12 }} labelStyle={{ color: "#8792a8" }} />}
            <Area type="monotone" dataKey="value" stroke={color} strokeWidth={2} fill={`url(#${gradId})`} isAnimationActive={false} />
            {events.map((e) => (
              <ReferenceDot key={e.date} x={e.date} y={e.value} r={5} fill={e.event === "spike" ? "#34d399" : "#f87171"} stroke="#0b0f1a" strokeWidth={2} />
            ))}
