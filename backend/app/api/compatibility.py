...
def list_ports(destination_only: bool = True, db: Session = Depends(get_db)) -> list[Port]:
    query = db.query(Port)
    if destination_only:
...
def list_vessel_classes(db: Session = Depends(get_db)) -> list[VesselClass]:
    return db.query(VesselClass).order_by(VesselClass.dwt_min).all()


@router.get("/matrix", response_model=list[MatrixRow])
def get_matrix(db: Session = Depends(get_db)) -> list[MatrixRow]:
    ports = db.query(Port).filter(Port.is_destination.is_(True)).order_by(Port.name).all()
