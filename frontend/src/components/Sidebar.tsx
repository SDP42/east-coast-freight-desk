...
import {
  Anchor, LayoutDashboard, TrendingUp, Compass, MapPinned, ShieldAlert, Calculator, FlaskConical, Activity, LogOut, Map,
} from "lucide-react";
...
export default function Sidebar() {
  ...
  const focus = new Set(
    { procurement_manager: ["recommendation", "financial", "scenario"], chartering_analyst: ["markets", "forecast", "recommendation"],
    ...
    <aside className="flex min-h-screen w-64 shrink-0 flex-col gap-6 border-r border-border-soft bg-ink/80 p-5 backdrop-blur-xl">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <h1 className="text-sm font-semibold leading-snug text-white">East Coast Freight Desk</h1>
        </div>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-all ${
                isActive ? "bg-panel-light text-white shadow-[inset_0_0_0_1px_rgba(34,211,238,0.25)]" : "text-muted hover:bg-panel hover:text-white"
              }`
            }
          >
            {({ isActive }) => (
      ...
      <div className="mt-auto space-y-3">
        <div className="rounded-lg border border-border-soft bg-panel p-3">
          ...
          <button
            onClick={logout}
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-md border border-border-soft py-1.5 text-xs text-muted transition hover:border-down/40 hover:text-down"
          >
