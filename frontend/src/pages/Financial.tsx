      ...
      <div className="p-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-5">
          <label className="text-xs text-muted">
            Index
            <select className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={indexName} onChange={(e) => setIndexName(e.target.value)}>
              {INDICES.map((i) => <option key={i} value={i}>{i}</option>)}
            </select>
          </label>
          <label className="text-xs text-muted">
            Current $/t
            <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={rate} onChange={(e) => setRate(e.target.value)} />
          </label>
          <label className="text-xs text-muted">
            Tonnes/fixture
            <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={tonnesPerFixture} onChange={(e) => setTonnesPerFixture(e.target.value)} />
          </label>
          <label className="text-xs text-muted">
            # Fixtures
            <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={numFixtures} onChange={(e) => setNumFixtures(e.target.value)} />
          </label>
          <label className="text-xs text-muted">
            Days apart
            <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={intervalDays} onChange={(e) => setIntervalDays(e.target.value)} />
          </label>
        </div>

        <button onClick={run} disabled={loading} className="mt-4 rounded-md bg-cyan/90 px-4 py-2 text-sm font-medium text-navy transition hover:bg-cyan disabled:opacity-50">
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
              <div className="rounded-md border border-border-soft bg-panel-light/50 p-2">
                ...
                <p className="font-semibold text-white">${result.total_coa_cost_usd.toLocaleString()}</p>
              </div>
              <div className="rounded-md border border-border-soft bg-panel-light/50 p-2">
                <p className="text-muted">Spot total (forecast)</p>
                <p className="font-semibold text-white">${result.total_spot_cost_usd.toLocaleString()}</p>
              </div>
              <div className="rounded-md border border-border-soft bg-panel-light/50 p-2">
                <p className="text-muted">Spot volatility (±)</p>
                <p className="font-semibold text-white">${result.spot_cost_std_usd.toLocaleString()}</p>
              </div>
            </div>
          </motion.div>
        )}
    ...
    <SpotlightCard glowColor="52,211,153">
      <div className="p-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <label className="text-xs text-muted">
            Annual tonnes
            <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={annualTonnes} onChange={(e) => setAnnualTonnes(e.target.value)} />
          </label>
          <label className="text-xs text-muted">
            Assumed $/t
            <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={assumedRate} onChange={(e) => setAssumedRate(e.target.value)} />
          </label>
          <label className="text-xs text-muted">
            Captured %
            <input className="mt-1 w-full rounded-md border border-border-soft bg-panel-light px-2 py-1.5 text-sm text-white" value={capturedPct} onChange={(e) => setCapturedPct(e.target.value)} />
          </label>
        </div>

        <button onClick={run} disabled={loading} className="mt-4 rounded-md bg-up/90 px-4 py-2 text-sm font-medium text-navy transition hover:bg-up disabled:opacity-50">
